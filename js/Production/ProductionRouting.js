...
function OnUpdateClick(s, e) { //Add/Edit/Close button function
    ...
    if (x == 0) {
        ...
        var indicies = gvStepProcess.batchEditApi.GetRowVisibleIndices();
        boolvalstep = true;
        for (var i = 0; i < indicies.length; i++) {
            if (gvStepProcess.batchEditApi.IsNewRow(indicies[i])) {
                ...
                if (gvStepProcess.batchEditApi.IsDeletedRow(key))
                    var aa;
                else {
                    gvStepProcess.batchEditApi.ValidateRow(indicies[i]);
                    gvStepProcess.batchEditApi.StartEdit(indicies[i], gvStepProcess.GetColumnByField("SKUCode").index);
...
function calculate_header(s, e) {
    ...
    setTimeout(function () {
        ...
        var iProduct = gvStepProcess.batchEditApi.GetRowVisibleIndices();
        ...
        for (var i = 0; i < iProduct.length; i++) {
            if (gvStepProcess.batchEditApi.IsNewRow(iProduct[i])) {
...
function calculate_BOM(s, e) {
    ...
    setTimeout(function () {
        ...
        var iProd = gvStepProcess.batchEditApi.GetRowVisibleIndices();
...
function getSize(s, e) {
    ...
    var igv1 = gvStepProcess.batchEditApi.GetRowVisibleIndices();

    for (var g = 0; g < igv1.length; g++) {
        if (gvStepProcess.batchEditApi.IsNewRow(igv1[g])) {
            if (sizeArr.length == 0) {
...
function ShowSizeHorizontal(s, e) {
    ...
    var hIndex = gvStepProcess.batchEditApi.GetRowVisibleIndices();
...
function CallbackSize(s, e) {
    ...
    var query = "";
    var cnt = 0;
    var gvIndex = gvStepProcess.batchEditApi.GetRowVisibleIndices();
...
function checkGrid() {
    ...
    for (var w = 0; w < indicies.length; w++) {
        if (gvStepProcess.batchEditApi.IsNewRow(indicies[w])) {
